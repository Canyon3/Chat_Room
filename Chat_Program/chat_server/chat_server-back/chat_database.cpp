...
ChatDataBase::~ChatDataBase()
{
	mysql_close(mysql);//断开数据库
}

void ChatDataBase::my_database_connect(const char *name)//连接到制定数据库
{
...
int ChatDataBase::my_database_get_group_name(string *s)//获取数据库中的群组名称列表
{
...
void ChatDataBase::my_database_get_group_member(string name, string &s)//获取指定群组的成员列表
{
	char sql[1024] = {0};
...
void ChatDataBase::my_database_disconnect()//断开与数据库的连接
{
	mysql_close(mysql);
}

bool ChatDataBase::my_database_user_exist(string name)//检查指定用户是否存在于数据库中
{
...
void ChatDataBase::my_database_user_password(string name, string password)//创建指定用户的密码表，并设置密码
{
	char sql[128] = {0};
...
bool ChatDataBase::my_database_password_correct(string name, string password)//检查用户输入的密码是否正确
{
	char sql[128] = {0};
...
void ChatDataBase::my_database_get_friend_group(string name, string &f, string &g)//获取指定用户的好友列表和所在群组列表
{
	char sql[128] = {0};
